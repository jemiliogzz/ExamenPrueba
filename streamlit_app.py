...
def inject_custom_css():
    st.markdown("""
    <style>
        /* General styles with Snowflake blue theme */
        .main {
...
# Function to get questions from bank table
...
# Function to initialize shuffled answers
    ...
    if not preguntas:
        ...
        <div class="progress-container">
            <p style="color: #0277BD; font-weight: bold; margin-bottom: 10px;">
                Progress: Question {pregunta_idx + 1} of {total_preguntas}
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="exam-container">
            <div class="question-box">
                <div class="question-number">Question {pregunta_idx + 1} of {total_preguntas}</div>
...
if st.session_state.examen_completado:
    ...
    with st.expander("📋 View answer details", expanded=False):
        for idx, pregunta in enumerate(preguntas):
            ...
            st.write(f"**Question:** {pregunta.QUESTION}")
            st.write(f"**Your answer:** {respuesta_usuario}")
            st.write(f"**Correct answer:** {respuesta_correcta}")
            st.write(f"**Subdomain:** {pregunta.DOMAIN_ID}.{pregunta.SUBDOMAIN}")
            st.divider()
    ...
    with col2:
        if st.button("🔄 Restart Exam", use_container_width=True):
            ...
            st.session_state.pregunta_actual = 0
            st.session_state.respuestas_mezcladas = {}
            # Clear individual question states
